...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <div 
      className={`${isOpen ? 'w-64' : 'w-20'} fixed top-0 left-0 z-50 flex flex-col h-screen text-white transition-all duration-500 ease-in-out border-r shadow-2xl bg-taxi-black border-taxi-gray`}
    >
      {/* Header / Logo Section */}
      <div className="relative flex items-center justify-center h-16 py-2 overflow-hidden border-b border-taxi-gray bg-gradient-to-r from-taxi-black to-taxi-dark">
        {/* We keep both images in DOM but fade them in/out for smoothness */}
        <div className={`absolute transition-opacity duration-500 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
           <img 
            src="/header.png" 
            alt="Pranav Drop Taxi" 
            className="object-contain w-auto h-12 transform scale-125"
          />
        </div>
      ...
      <nav className="flex-1 py-6 space-y-2 overflow-x-hidden">
        {menuItems.map((item) => (
          ...
          >
            {/* Icon - Fixed width to prevent squashing */}
            <span className={`text-xl transition-all duration-300 ${isOpen ? 'mr-0' : ''}`}>
              {item.icon}
            </span>
            
            {/* Text Label - Smooth Width/Opacity Animation */}
            <span 
              className={`font-medium whitespace-nowrap overflow-hidden transition-all duration-500 ease-in-out
                ${isOpen 
                  ? 'w-40 ml-4 opacity-100 translate-x-0' 
      ...
      <div className="p-4 border-t border-taxi-gray">
        <button
          onClick={logout}
          className={`flex items-center justify-center w-full py-2 text-red-500 transition-all duration-300 rounded bg-red-600/10 hover:bg-red-600 hover:text-white group overflow-hidden ${
            !isOpen ? 'px-0' : ''
          }`}
          title={!isOpen ? 'Logout' : ''}
        >
